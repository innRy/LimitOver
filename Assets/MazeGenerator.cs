...
{
    [Header("迷路の設定")]
    [Tooltip("通路の数。16に設定すると1辺の物理的な長さが49になります")]
    public int pathCount = 16;

    [Header("各要素の高さ")]
    public float outerWallHeight = 2f; // 外枠の高さ
    public float innerWallHeight = 2f; // 内壁の高さ
    public float pathHeight = 1f;      // 通路の高さ

    [Header("取得用座標（他のスクリプトから参照可能）")]
        ...
        for (int x = 0; x < logicalSize; x++)
        {
            for (int z = 0; z < logicalSize; z++)
            {
                ...
                else // 通路・スタート・ゴールの場合
                {
                    ...
                    else if (x == logicalSize - 2 && z == logicalSize - 2) // 迷路の右上
                    {
                        path.name = "GoalCube";
